...
const ArticleComponent = ({ article, currentActiveTab }: ArticleComponentType) => {
    ...
    return <a   className={`${isTagPresent ? 'flex' : 'hidden'} group col-span-4 md:col-span-2 min-h-[170px] flex-col justify-between bg-[#1D1C1C] px-[18px] py-[14px] rounded-[12px] hover:bg-primary-red`} 
                href={`${article?.link}`}
                target="_blank"
                rel="noreferrer"
                >
